...
## 🔄 Redirection de `cookingrecipe.delhomme.ovh` vers `cookingrecipes.delhomme.ovh`

### 📋 Situation Actuelle

Vous avez déjà configuré dans **Nginx Proxy Manager** :
- **Proxy Host principal** : `cookingrecipes.delhomme.ovh` → `http://cookingrecipes-frontend:8080` (avec SSL)
- **Proxy Host API** : `api.cookingrecipes.delhomme.ovh` → `http://cookingrecipes-api:7272` (avec SSL)
...
- Accédez à `https://ngin.delhomme.ovh` (ou votre URL NPM)
...
- **Forward Hostname/IP** : `cookingrecipes.delhomme.ovh`
...
```nginx
return 301 https://cookingrecipes.delhomme.ovh$request_uri;
```
...
1. **Frontend** : `cookingrecipes.delhomme.ovh` → `http://cookingrecipes-frontend:8080` (SSL ✅)
2. **API** : `api.cookingrecipes.delhomme.ovh` → `http://cookingrecipes-api:7272` (SSL ✅)
3. **Redirection** : `cookingrecipe.delhomme.ovh` → `https://cookingrecipes.delhomme.ovh:443` (SSL ✅)
   - Avec dans Advanced : `return 301 https://cookingrecipes.delhomme.ovh$request_uri;`

#### ✅ Dans OVH DNS (3 enregistrements A) :
1. `cookingrecipes.delhomme.ovh.` → `95.111.227.204`
2. `api.cookingrecipes.delhomme.ovh.` → `95.111.227.204`
3. `cookingrecipe.delhomme.ovh.` → `95.111.227.204`
...
nslookup cookingrecipe.delhomme.ovh
# Devrait retourner : 95.111.227.204

nslookup cookingrecipes.delhomme.ovh
# Devrait retourner : 95.111.227.204

nslookup api.cookingrecipes.delhomme.ovh
# Devrait retourner : 95.111.227.204
```
...
curl -I http://cookingrecipe.delhomme.ovh

# Tester la redirection HTTPS
curl -I https://cookingrecipe.delhomme.ovh

# Vous devriez voir :
# HTTP/1.1 301 Moved Permanently
# Location: https://cookingrecipes.delhomme.ovh/
```

### Test 3 : Vérifier dans le Navigateur

1. **Ouvrez** `https://cookingrecipe.delhomme.ovh` dans votre navigateur
2. **Vous devriez être automatiquement redirigé** vers `https://cookingrecipes.delhomme.ovh`
...
1. **Ouvrez** `https://cookingrecipes.delhomme.ovh` directement
...
curl https://api.cookingrecipes.delhomme.ovh/health
...
3. **Redirection** (NOUVEAU) :
   ...
   - Forward : `https://cookingrecipes.delhomme.ovh:443`
   - SSL : ✅ Activé
   - Custom Nginx : `return 301 https://cookingrecipes.delhomme.ovh$request_uri;`
...
- Si vous ajoutez `cookingrecipe.delhomme.ovh` dans les "Domain Names" du proxy principal, les deux domaines pointeront vers le même conteneur
...
3. **Trouvez l'enregistrement pour `cookingrecipe.delhomme.ovh`**
4. **Modifiez l'enregistrement** :
   - Changez le type en **CNAME** (si ce n'est pas déjà le cas)
   - Pointez vers `cookingrecipes.delhomme.ovh`
...
3. **Vérifiez la configuration "Custom Nginx Configuration"** :
   - Doit contenir exactement : `return 301 https://cookingrecipes.delhomme.ovh$request_uri;`
   - Pas d'autres lignes
...
Une fois configuré, tous les accès à `cookingrecipe.delhomme.ovh` seront automatiquement redirigés vers `cookingrecipes.delhomme.ovh` avec une redirection 301 permanente.
...
2. **Testez la redirection** dans votre navigateur :
   - Ouvrez `https://cookingrecipe.delhomme.ovh`
   - Vous devriez être redirigé vers `https://cookingrecipes.delhomme.ovh`
3. **C'est tout !** 🎉
...
3. **Vérifiez les logs** dans Nginx Proxy Manager :
   ...
   - Regardez les requêtes vers `cookingrecipe.delhomme.ovh`

4. **Vérifiez que le DNS est bien propagé** :
   ```bash
   nslookup cookingrecipe.delhomme.ovh
   # Doit retourner : 95.111.227.204
   ```
