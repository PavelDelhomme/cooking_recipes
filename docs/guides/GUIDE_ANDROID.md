...
1. **Connectez votre téléphone via USB**
   ...
   adb devices
   ```
   Vous devriez voir : `YOUR_DEVICE_ID    device`
...
adb devices
```
**Résultat attendu** : `YOUR_DEVICE_ID    device`
...
```bash
adb -s YOUR_DEVICE_ID shell pm list packages | grep cooking
```
**Résultat attendu** : `package:com.example.cooking_recipe.cookingrecipe`
...
```bash
adb -s YOUR_DEVICE_ID logcat | grep -i "flutter\|cooking\|error\|exception"
```
...
```bash
adb -s YOUR_DEVICE_ID shell am start -n com.example.cooking_recipe.cookingrecipe/.MainActivity
```
...
adb -s YOUR_DEVICE_ID uninstall com.example.cooking_recipe.cookingrecipe
...
**Vérifier les logs** :
```bash
adb -s YOUR_DEVICE_ID logcat -d | tail -100
```
...
```bash
adb -s YOUR_DEVICE_ID logcat | grep -i flutter
```

### Voir tous les logs
```bash
adb -s YOUR_DEVICE_ID logcat
```
...
```bash
adb -s YOUR_DEVICE_ID shell am force-stop com.example.cooking_recipe.cookingrecipe
adb -s YOUR_DEVICE_ID shell am start -n com.example.cooking_recipe.cookingrecipe/.MainActivity
```

### Vérifier la version installée
```bash
adb -s YOUR_DEVICE_ID shell dumpsys package com.example.cooking_recipe.cookingrecipe | grep versionName
```

### Prendre une capture d'écran
```bash
adb -s YOUR_DEVICE_ID shell screencap -p /sdcard/screenshot.png
adb -s YOUR_DEVICE_ID pull /sdcard/screenshot.png
```
